...
//			System.out.println("CrailBufferedOutputStream: crailFS.allocateBuffer()"+(end-start));
...
//		long prepareStart=0, prepareEnd=0, writeStart=0, writeEnd=0, process0StartPrepare=0, process0EndPrepare=0, process1StartPrepare=0, process1EndPrepare=0;
...
//			LOG.info("LQL debug arg dataBuf remaining "+len);

			while (len > 0) {
//				prepareStart=System.nanoTime();
//				process0StartPrepare=System.nanoTime();
				CrailBuffer slice = getSlice();
//				process0EndPrepare=System.nanoTime();
//				process1StartPrepare=System.nanoTime();
				int bufferRemaining = Math.min(len, slice.remaining());
//				LOG.info("LQL debug len "+len+" slice.remaining() "+slice.remaining()+" bufferRemaining "+bufferRemaining);
				int oldLimit = dataBuf.limit();
//				LOG.info("LQL debug oldLimit "+oldLimit+" dataBuf.position() "+dataBuf.position());
				dataBuf.limit(dataBuf.position() + bufferRemaining);
				slice.put(dataBuf);
				dataBuf.limit(oldLimit);
				len -= bufferRemaining;
//				LOG.info("after len "+len+" before position "+position);
				position += bufferRemaining;
//				process1EndPrepare=System.nanoTime();
//				prepareEnd=System.nanoTime();
//				LOG.info("after position "+position);

//				writeStart=System.nanoTime();
				syncSlice();
//				writeEnd=System.nanoTime();

//				System.out.println("write prepare time: "+(prepareEnd-prepareStart));
//				System.out.println("write process0 time: "+(process0EndPrepare-process0StartPrepare));
//				System.out.println("write process1 time: "+(process1EndPrepare-process1StartPrepare));
//				System.out.println("write sync time: "+(writeEnd-writeStart));
			}
		} catch (Exception e) {
			throw new IOException(e);
		}
