...
public class CoreDataStore extends CrailStore {
    ...
    // record redundant in memory file
//	private ConcurrentHashMap<String, RedundancyType> redundancyFiles;
...
//		RedundancyMonitor redundancyMonitor=RedundancyMonitor.getInstance();
//		this.redundancyFiles=redundancyMonitor.redundancyFiles;
    ...
    public Upcoming<CrailNode> createReplicasFile(String path, CrailNodeType type, CrailStorageClass storageClass, CrailLocationClass locationClass, boolean enumerable, RedundancyType redundancyType) throws Exception {
        FileName primaryName = new FileName(path);
        FileName[] slavesName = new FileName[CrailConstants.REPLICATION_FACTOR];
        for (int i = 0; i < CrailConstants.REPLICATION_FACTOR; i++) {
            slavesName[i] = new FileName(path + "LQL" + i);

//            System.out.println("slavesName: "+path+"LQL"+i);
        }

        LOG.info("createNode replicas: name " + path + ", type " + type + ", storageAffinity " + storageClass + ", locationAffinity " + locationClass);

        RpcFuture<RpcCreateFile> primaryFileRes = rpcConnection.createFile(primaryName, type, storageClass.value(), locationClass.value(), enumerable);
        List<Future<RpcCreateFile>> slavesFileRes = new ArrayList<>();
        for (int i = 0; i < CrailConstants.REPLICATION_FACTOR; i++) {
            slavesFileRes.add(rpcConnection.createFile(slavesName[i], type, storageClass.value(), locationClass.value(), enumerable));
        }

        return new CreateReplicasNodeFuture(this, path, type, primaryFileRes, slavesFileRes, redundancyType);
    }

    CoreNode _createReplicasNode(String path, String[] slavesPath, CrailNodeType type, RpcCreateFile primaryFileRes, RpcCreateFile[] slavesFileRes, RedundancyType redundancyType) throws Exception {
        CoreNode primaryNode = new ReplicasFile(_createNode(path, type, primaryFileRes));
        CoreNode[] slavesNode = new CoreNode[CrailConstants.REPLICATION_FACTOR];
        for (int i = 0; i < CrailConstants.REPLICATION_FACTOR; i++) {
            slavesNode[i] = _createNode(slavesPath[i], type, slavesFileRes[i]);

//            System.out.println("slave file "+i+" has been created! File path: "+slavesNode[i].getPath());
        }
        ((ReplicasFile) primaryNode).setSlavesFile(slavesNode);

//		redundancyFiles.put(path, redundancyType);
//		System.out.println("redundancyFile "+redundancyFiles);
...
//		long initStart=System.nanoTime();
        ...
        if (fileInfo.getType() != type) {
            System.out.println("fileInfo.getType() " + fileInfo.getType() + " type " + type);
...
//        System.out.println("_createNode check: "+(checkEnd-checkStart));
//        System.out.println("_createNode init: "+(initEnd-initStart));
//        System.out.println("_createNode create: "+(createEnd-createStart));
//        System.out.println("_createNode save: "+(saveEnd-saveStart));
...
//			System.out.println("_lookupNode redundancyFiles "+redundancyFiles);
//			if(redundancyFiles.containsKey(path)){
//				System.out.println(path+"is a redundant file, redundant type is "+redundancyFiles.get(path));
//			}

            if ((CrailConstants.REDUNDANCY_TYPE.equals("replicas") || CrailConstants.REDUNDANCY_TYPE.equals("erasurecoding")) && !path.contains("LQL")) {
//				System.out.println("CoreDataStore replicasFile or erasureCodingFile!");
                node = new ReplicasFile(node);
...
//				System.out.println("Before set slavesFile!");
                CoreNode[] slavesFile = new CoreNode[CrailConstants.REPLICATION_FACTOR];
                for (int i = 0; i < CrailConstants.REPLICATION_FACTOR; i++) {
                    slavesFile[i] = (CoreNode) this.lookup(path + "LQL" + i).get();
                }
//				System.out.println("After set slavesFile!");
                ((ReplicasFile) node).setSlavesFile(slavesFile);
//				System.out.println("Finish set slavesFile!");
            }
        }
        return node;
    }
    ...
    public void deleteRedundantFile(CrailFile redundantFile) throws Exception {
        if (!(redundantFile instanceof ReplicasFile)) {
            throw new Exception("Error delete! The file " + redundantFile.getPath() + " is not a ReplicasFile!");
        }

        ReplicasFile tmp = (ReplicasFile) redundantFile;
        String primaryPath = tmp.getPath();
        CoreFile[] slavesFile = tmp.getSlavesFile();
        for (int i = 0; i < slavesFile.length; i++) {
            this.delete(slavesFile[i].getPath(), false).get().syncDir();
        }

        this.delete(primaryPath, false).get().syncDir();
    }
    ...
    CoreReplicasOutputStream getReplicasOutputStream(CoreNode file, long writeHint) throws Exception {
        CoreReplicasOutputStream replicasOutputStream = new CoreReplicasOutputStream(file, streamCounter.incrementAndGet(), writeHint);
        openOutputStreams.put(replicasOutputStream.getStreamId(), replicasOutputStream);
    ...
    CoreErasureCodingInputStream getErasureCodingInputStream(CoreNode file, long readHint) throws Exception {
        CoreErasureCodingInputStream erasureCodingInputStream = new CoreErasureCodingInputStream(file, streamCounter.incrementAndGet(), readHint);
        openInputStreams.put(erasureCodingInputStream.getStreamId(), erasureCodingInputStream);
        ...
        return erasureCodingInputStream;
    }

    CoreErasureCodingInputStream getDegradeErasureCodingInputStream(CoreNode file, long readHint, int[] degradeIndices) throws Exception {
        CoreErasureCodingInputStream erasureCodingInputStream = new CoreErasureCodingInputStream(file, streamCounter.incrementAndGet(), readHint, degradeIndices);
        openInputStreams.put(erasureCodingInputStream.getStreamId(), erasureCodingInputStream);
