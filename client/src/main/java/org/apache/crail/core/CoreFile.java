...
public class CoreFile extends CoreNode implements CrailFile, CrailKeyValue {
	...
	public CrailInputStream getErasureCodingDirectInputStream(long readHint) throws Exception{
		if (fileInfo.getType().isDirectory()){
			throw new Exception("Cannot open stream for directory");
		}

		return super.getErasureCodingInputStream(readHint);
	}

	public CrailInputStream getDegradeErasureCodingDirectInputStream(long readHint, int[] degradeIndices) throws Exception{
		if (fileInfo.getType().isDirectory()){
			throw new Exception("Cannot open stream for directory");
		}

		return super.getDegradeErasureCodingInputStream(readHint, degradeIndices);
	}
	...
	public synchronized CrailOutputStream getReplicasDirectOutputStream(long writeHint) throws Exception {
		if (fileInfo.getType().isDirectory()){
			throw new Exception("Cannot open stream for directory");
		}
		if (fileInfo.getToken() == 0){
		...
		return super.getReplicasOutputStream(writeHint);
	}
